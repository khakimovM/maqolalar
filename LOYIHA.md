...
Har bir maqolada:
- `title` — sarlavha
- `slug` — URL uchun avtomatik yasaladi (masalan: `"Python Asoslari"` → `python-asoslari`), unique
- `content` — Tiptap JSON format (rich text)
- `coverImage` — muqova rasmi (optional)
- `images` — maqola ichidagi rasmlar (ArticleImage modeli)
...
2. `POST /auth/verify-reset-otp` — OTP tekshiriladi → reset token qaytaradi
3. `POST /auth/reset-password` — yangi parol + reset token
...
**RefreshToken**
- `token` (unique), `userId`, `expiresAt` — logout va multi-device uchun DB da saqlanadi

**Category**
- `name` (unique), `slug` (unique) — SuperAdmin tomonidan boshqariladi

**Article**
- `title`, `slug` (unique), `content` (Json — Tiptap), `coverImage`, `type` (FREE/PREMIUM), `status` (DRAFT/PUBLISHED/ARCHIVED), `publishedAt`, `authorId`, `categoryId`

**ArticleImage**
- `articleId`, `url`, `altText`, `order` — maqola ichidagi rasmlar

**Like**
- `userId`, `articleId` — unique(userId + articleId) — bir marta layk

**Comment**
- `content`, `isDeleted` (soft delete), `authorId`, `articleId`, `parentId` (null = top-level, value = reply) — o'ziga-o'zi self-relation
...
**Notification**
- `userId` (kimga), `type` (NEW_COMMENT/NEW_REPLY/NEW_LIKE/COMMENT_SPAM), `referenceId` (maqola yoki comment ID), `isRead`
...
| PATCH | /categories/:id | SUPERADMIN |
...
| PATCH | /admin/admins/:id/deactivate | SUPERADMIN |
